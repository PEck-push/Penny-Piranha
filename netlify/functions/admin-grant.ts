...
//
// Body: { playerId, tokens?, accessoryId? }
//   tokens     — absolut zu addierender Betrag (kann negativ sein für Abzug).
//   accessoryId — ID eines Accessoires, das in unlockedOverlays ergänzt wird.
// Beides optional, mindestens eines muss gesetzt sein.

interface Body { playerId: string; tokens?: number; accessoryId?: string; }
...
export default async (req: Request, _ctx: Context) => {
  ...
  if (tokensDelta === 0 && !accessoryId) return json({ error: 'Kein tokens oder accessoryId angegeben.' }, 400);
  ...
  try {
    ...
    if (tokensDelta !== 0) upd.tokens = FieldValue.increment(tokensDelta);
